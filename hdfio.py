    ...
    for key, value in idiobj.header.items():
        ...
        if is_comment or is_table or is_basic or is_hdf:
            ...
            if attrVal.dtype.kind in {'U', 'S'}:
                attrVal = np.array([np.string_(value)])
                
            print(repr(attrVal))
            
            hduobj.attrs[key] = attrVal
            hduobj.attrs[key+"_COMMENT"] = np.array([np.string_(comment)])
    return hduobj
    ...
    with h5py.File(outfile, mode='w') as h:
        ...
        for gkey, gdata in idi_hdu.items():
            ...
            if idi_hdu[gkey].comment:
                gg.create_dataset("COMMENT", data=np.string_(idi_hdu[gkey].comment))
            if idi_hdu[gkey].history:
                print(repr(idi_hdu[gkey].history))
                print(type(idi_hdu[gkey].history))
                gg.create_dataset("HISTORY", data=np.string_(idi_hdu[gkey].history))
